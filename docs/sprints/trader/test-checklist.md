...
5. Manual verification:
   ...
   curl --cert tls/wl-a/wl_a.p12:changeit --cert-type P12 -k \
     https://localhost:8443/cpservice/healthz
   ```

## 2. Control Plane Services
1. Build and start core stack:
   ```bash
   docker compose up -d postgres cpservice gateway
   ```
...
3. Issue a WL:
   ...
   curl --cert tls/wl-a/wl_a.p12:changeit --cert-type P12 -k \
     ...
     https://localhost:8443/cpservice/wls
   ```
...
6. Rotate WL key and fetch JWKS:
   ...
   curl --cert tls/wl-a/wl_a.p12:changeit --cert-type P12 -k \
     https://localhost:8443/cpservice/.well-known/jwks.json
   ```
7. Relationships:
   ```bash
   curl --cert tls/wl-a/wl_a.p12:changeit --cert-type P12 -k \
     ...
     https://localhost:8443/cpservice/relationships/<WL_A>/<WL_B>
   ```
8. Imbalance signals + ack:
   ...
   SIGNAL_ID=$(curl --cert tls/wl-a/wl_a.p12:changeit --cert-type P12 -k \
     ...
     https://localhost:8443/cpservice/imbalance/signals | jq -r '.id')
   curl --cert tls/wl-a/wl_a.p12:changeit --cert-type P12 -k \
     -X POST -H 'Content-Type: application/json' \
     -d '{"acknowledgedBy":"wl-agent"}' \
     https://localhost:8443/cpservice/imbalance/signals/$SIGNAL_ID/ack
   ```
   ...
   ```bash
   # Ensure target WL was registered with gatewayUrl set (use POST /wls)
   # Re-send the signal and confirm dispatchStatus == sent/acknowledged in response
   ```
9. Trade-balance report:
   ```bash
   curl --cert tls/wl-a/wl_a.p12:changeit --cert-type P12 -k \
     https://localhost:8443/cpservice/reports/trade-balance
   ```
...
2. Start admin UI with compose:
   ...
   ```
3. Access via browser: `https://localhost:8443/admin/` (ensure browser trusts gateway cert or disable verification). Use WL client cert for mTLS.
4. Validate tabs:
...
1. Rate limiting: issue rapid requests with client cert to `/cpservice/healthz` and confirm `429` after burst.
   ```bash
   for i in {1..40}; do curl --cert tls/wl-a/wl_a.p12:changeit --cert-type P12 -k -o /dev/null -s https://localhost:8443/cpservice/healthz; done
   ```
2. WAF check: request with suspicious string (e.g., `/cpservice/wls?foo=../etc/passwd`) should return `403`.

## 5. Observability Spot Checks
1. Logs: `docker compose logs cpservice` should include `[cid:...]` and redact emails/phones (`[EMAIL_REDACTED]`).
...
- Import `cpservice/docs/postman/control-plane.postman_collection.json` and exercise each request with client cert authentication.
